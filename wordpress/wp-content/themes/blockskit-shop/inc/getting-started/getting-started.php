...
function bk_widgets_backend_enqueue()
{
    ...
    $translation = array(
        'btn_text' => esc_html__( 'Processing...', 'blockskit-shop' ),
...
class Bk_Notice_Handler {
    ...
    public function bk_theme_info_welcome_admin_notice() {
        if ( is_admin() && !get_user_meta( get_current_user_id(), 'gs_notice_dismissed' ) ){
            ...
            if ( is_plugin_active( 'blockskit-import/blockskit-import.php' ) && is_plugin_active( 'advanced-import/advanced-import.php' ) ) {
                echo ('<div class="fs-notice success"><p>'.esc_html__( 'Essential plugins are added successfully. You may dismiss this notice now.', 'blockskit-shop' ).'</p></div>');
            }else{
            echo ( '<p><strong>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s.', 'blockskit-shop' ), $this->theme_name ) . '</strong></p><p class="plugin-notice">'.esc_html__( 'By clicking "Get Started," you will install and activate Blockskit Import & Advanced Import plugins to prepare your demo import.', 'blockskit-shop' ).'</p><p><a href="#" class="bk-install-plugins button button-primary">' . sprintf( esc_html__( 'Get started with %s','blockskit-shop' ), $this->theme_name ) . '</a></p>' );
            }
	...
	public function bk_getting_started() {
        ...
        if ( ! current_user_can( 'install_plugins' ) ) {
            $status['errorMessage'] = __( 'Sorry, you are not allowed to install plugins on this site.', 'blockskit-shop' );
        ...
        if( $request == 2 && strpos( $slug, 'blockskit-shop' ) === false ){
            wp_send_json_error( );
        }
        ...
        if ( is_wp_error( $result ) ) {
            ...
            $status['errorMessage'] = __( 'Unable to connect to the filesystem. Please confirm your credentials.', 'blockskit-shop' );
