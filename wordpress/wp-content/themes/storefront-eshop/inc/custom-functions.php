...
if( !function_exists( 'storefront_eshop_fonts_url' ) ) :

    //Google Fonts URL
    function storefront_eshop_fonts_url(){
...
if ( ! function_exists( 'storefront_eshop_sub_menu_toggle_button' ) ) :
    ...
    function storefront_eshop_sub_menu_toggle_button( $args, $item, $depth ) {

        // Add sub menu toggles to the main menu with toggles
        if ( $args->theme_location == 'storefront-eshop-primary-menu' && isset( $args->show_toggles ) ) {
            ...
            if ( in_array( 'menu-item-has-children', $item->classes ) ) {
                ...
                $args->after .= '<button type="button" class="theme-aria-button submenu-toggle" data-toggle-target="' . $toggle_target_string . '" data-toggle-type="slidetoggle" data-toggle-duration="250" aria-expanded="false"><span class="btn__content" tabindex="-1"><span class="screen-reader-text">' . esc_html__( 'Show sub menu', 'storefront-eshop' ) . '</span>' . storefront_eshop_get_theme_svg( 'chevron-down' ) . '</span></button>';
        ...
        }elseif( $args->theme_location == 'storefront-eshop-primary-menu' ) {

            if ( in_array( 'menu-item-has-children', $item->classes ) ) {

                $args->before = '<div class="link-icon-wrapper">';
                $args->after  = storefront_eshop_get_theme_svg( 'chevron-down' ) . '</div>';
...
if ( ! function_exists( 'storefront_eshop_the_theme_svg' ) ):
    ...
    function storefront_eshop_the_theme_svg( $svg_name, $return = false ) {

        if( $return ){

            return storefront_eshop_get_theme_svg( $svg_name ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in storefront_eshop_get_theme_svg();.

        }else{

            echo storefront_eshop_get_theme_svg( $svg_name ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in storefront_eshop_get_theme_svg();.
...
if ( ! function_exists( 'storefront_eshop_get_theme_svg' ) ):
    ...
    function storefront_eshop_get_theme_svg( $svg_name ) {

        // Make sure that only our allowed tags and attributes are included.
        $svg = wp_kses(
            Storefront_Eshop_SVG_Icons::get_svg( $svg_name ),
            array(
                'svg'     => array(
                    'class'       => true,
                    'xmlns'       => true,
...
if( !function_exists( 'storefront_eshop_post_category_list' ) ) :
    ...
    // Post Category List.
    function storefront_eshop_post_category_list( $select_cat = true ){
        ...
        if( $select_cat ){
            ...
            $post_cat_cat_array[''] = esc_html__( '-- Select Category --','storefront-eshop' );
...
if( !function_exists('storefront_eshop_single_post_navigation') ):
    ...
    function storefront_eshop_single_post_navigation(){
        ...
        if( $twp_navigation_type == '' || $twp_navigation_type == 'global-layout' ){
            $twp_navigation_type = get_theme_mod('twp_navigation_type', $storefront_eshop_default['twp_navigation_type']);
        }
            ...
            if( $twp_navigation_type == 'theme-normal-navigation' ){ ?>
                ...
                <div class="navigation-wrapper">
                    ...
                    the_post_navigation(array(
                        'prev_text' => '<span class="arrow" aria-hidden="true">' . storefront_eshop_the_theme_svg('arrow-left',$return = true ) . '</span><span class="screen-reader-text">' . esc_html__('Previous post:', 'storefront-eshop') . '</span><span class="post-title">%title</span>',
                        'next_text' => '<span class="arrow" aria-hidden="true">' . storefront_eshop_the_theme_svg('arrow-right',$return = true ) . '</span><span class="screen-reader-text">' . esc_html__('Next post:', 'storefront-eshop') . '</span><span class="post-title">%title</span>',
                    )); ?>
                </div>
                <?php
                    ...
                    <div class="column column-9">
                        <div class="footer-credits">
                            ...
                            <div class="footer-copyright">
                                ...
                                <?php
                                $footer_copyright_text = wp_kses_post( get_theme_mod( 'footer_copyright_text', $storefront_eshop_default['footer_copyright_text'] ) );
                                    echo esc_html( $footer_copyright_text );
                                    echo '<br>';
                                    echo esc_html__('Theme: ', 'storefront-eshop') . '<a href="' . esc_url('https://omegathemes.com/wordpress/free-storefront-wordpress-theme/') . '" title="' . esc_attr__('Storefront Eshop ', 'storefront-eshop') . '" target="_blank"><span>' . esc_html__('Storefront Eshop ', 'storefront-eshop') . '</span></a>' . esc_html__('By ', 'storefront-eshop') . '  <span>' . esc_html__('OMEGA ', 'storefront-eshop') . '</span>';
                    ...
                    <div class="column column-3 align-text-right">
                        <a class="to-the-top" href="#site-header">
                            <span class="to-the-top-long">
                                <?php
                                printf( esc_html__( 'To the Top %s', 'storefront-eshop' ), '<span class="arrow" aria-hidden="true">&uarr;</span>' );
                                ?>
                            </span>
                            <span class="to-the-top-short">
                                <?php
                                printf( esc_html__( 'Up %s', 'storefront-eshop' ), '<span class="arrow" aria-hidden="true">&uarr;</span>' );
                                ?>
                            </span>
                        </a>
...
if( !function_exists( 'storefront_eshop_main_slider' ) ) :
    ...
    function storefront_eshop_main_slider(){
        ...
        $homepage_section_shipping_title = esc_html( get_theme_mod( 'storefront_eshop_homepage_section_shipping_title',
        $storefront_eshop_defaults['storefront_eshop_homepage_section_shipping_title'] ) );

        $homepage_section_shipping_text = esc_html( get_theme_mod( 'storefront_eshop_homepage_section_shipping_text',
        $storefront_eshop_defaults['storefront_eshop_homepage_section_shipping_text'] ) );

        $homepage_section_exchange_title = esc_html( get_theme_mod( 'storefront_eshop_homepage_section_exchange_title',
        $storefront_eshop_defaults['storefront_eshop_homepage_section_exchange_title'] ) );

        $homepage_section_exchange_text = esc_html( get_theme_mod( 'storefront_eshop_homepage_section_exchange_text',
        $storefront_eshop_defaults['storefront_eshop_homepage_section_exchange_text'] ) );

        $homepage_section_support_title = esc_html( get_theme_mod( 'storefront_eshop_homepage_section_support_title',
        $storefront_eshop_defaults['storefront_eshop_homepage_section_support_title'] ) );

        $homepage_section_support_text = esc_html( get_theme_mod( 'storefront_eshop_homepage_section_support_text',
        $storefront_eshop_defaults['storefront_eshop_homepage_section_support_text'] ) );

        $homepage_section_shopping_title = esc_html( get_theme_mod( 'storefront_eshop_homepage_section_shopping_title',
        $storefront_eshop_defaults['storefront_eshop_homepage_section_shopping_title'] ) );

        $homepage_section_shopping_text = esc_html( get_theme_mod( 'storefront_eshop_homepage_section_shopping_text',
        ...
        if( $storefront_eshop_header_banner ){
          ...
          $banner_query = new WP_Query( array('post_type' => 'post', 'posts_per_page' => 4,'post__not_in' => get_option("sticky_posts"), 'category_name' => esc_html( $storefront_eshop_header_banner_cat ) ) );
...
if (!function_exists('storefront_eshop_post_format_icon')):
    ...
    function storefront_eshop_post_format_icon() {
        ...
        $title = '';
        if( $format == 'video' ){
            $icon = storefront_eshop_get_theme_svg( 'video' );
            $title = esc_html__('Video','storefront-eshop');
        }elseif( $format == 'audio' ){
            $icon = storefront_eshop_get_theme_svg( 'audio' );
            $title = esc_html__('Audio','storefront-eshop');
        }elseif( $format == 'gallery' ){
            $icon = storefront_eshop_get_theme_svg( 'gallery' );
            $title = esc_html__('Gallery','storefront-eshop');
        }elseif( $format == 'quote' ){
            $icon = storefront_eshop_get_theme_svg( 'quote' );
            $title = esc_html__('Quote','storefront-eshop');
        }elseif( $format == 'image' ){
            $icon = storefront_eshop_get_theme_svg( 'image' );
            $title = esc_html__('Image','storefront-eshop');
        }
        
        if (!empty($icon)) { ?>
            <div class="theme-post-format">
                <span class="post-format-icom"><?php echo storefront_eshop_svg_escape($icon); ?></span>
...
if ( ! function_exists( 'storefront_eshop_svg_escape' ) ):
     ...
     */
    function storefront_eshop_svg_escape( $input ) {
...
if( !function_exists( 'storefront_eshop_sanitize_sidebar_option_meta' ) ) :
    ...
    function storefront_eshop_sanitize_sidebar_option_meta( $input ){
            ...
            <div class="theme-product-block">
                <div class="wrapper">
                    ...
                    <div class="product-image">
                        <?php $loop = new WP_Query( $storefront_eshop_args );
